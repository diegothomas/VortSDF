...
class Runner:
    ...
    def prep_CVT(self):
        ...
        _, idx = KDtree.query(self.sites, k=1)
    ...
    def train(self, data_name, K_NN = 24, verbose = True, is_continue = False):
        ...
        if not hasattr(self, 'tet32'):
            ...
            ext_flag[sites[:,0] < self.visual_hull[0] - 10.5] = 1
            ext_flag[sites[:,1] < self.visual_hull[1] - 10.5] = 1
            ext_flag[sites[:,2] < self.visual_hull[2] - 10.5] = 1
            ext_flag[sites[:,0] > self.visual_hull[3] + 10.5] = 1
            ext_flag[sites[:,1] > self.visual_hull[4] + 10.5] = 1
            ext_flag[sites[:,2] > self.visual_hull[5] + 10.5] = 1
        ...
        outside_flag[sites[:,0] < self.visual_hull[0] + (self.visual_hull[3]-self.visual_hull[0])/(2*res)] = 1
        outside_flag[sites[:,1] < self.visual_hull[1] + (self.visual_hull[4]-self.visual_hull[1])/(2*res)] = 1
        outside_flag[sites[:,2] < self.visual_hull[2] + (self.visual_hull[5]-self.visual_hull[2])/(2*res)] = 1
        outside_flag[sites[:,0] > self.visual_hull[3] - (self.visual_hull[3]-self.visual_hull[0])/(2*res)] = 1
        outside_flag[sites[:,1] > self.visual_hull[4] - (self.visual_hull[4]-self.visual_hull[1])/(2*res)] = 1
        outside_flag[sites[:,2] > self.visual_hull[5] - (self.visual_hull[5]-self.visual_hull[2])/(2*res)] = 1

        ext_flag = np.zeros(self.sites.shape[0], np.int32)
        ...
        if not is_continue:
            ...
            self.s_start = 10.0
            self.inv_s = 0.1
            self.sigma = 0.1 #min((self.visual_hull[3]-self.visual_hull[0]), (self.visual_hull[4]-self.visual_hull[1]), (self.visual_hull[5]-self.visual_hull[2]))/(2.0*res)
            step_size = 0.01
            self.w_g = 0.01
        ...
        for iter_step in tqdm(range(self.end_iter)):
            ...
            if self.double_net:
                if self.position_encoding:
                    ...
                    #, 1.0+self.out_sdf[:nb_samples,:2]/(3.0*self.sigma)
                    rgb_feat = torch.cat([xyz_emb, viewdirs_emb, colors_feat, (1.0+self.out_grads[:nb_samples,:3])/2.0, self.out_feat[:nb_samples,:32]], -1)
                else:
                ...
                if self.position_encoding:
                    rgb_feat = torch.cat([xyz_emb, viewdirs_emb, self.out_sdf[:nb_samples,:2], self.out_grads[:nb_samples,:3], self.out_feat[:nb_samples,:32]], -1)
                else:
            ...
            if self.double_net:
                loss = (color_fine_loss + 0.5*color_coarse_loss) #/ (mask_sum + 1.0e-5)
            else:
            ...
            if self.double_net:
                if self.position_encoding:
                    ...
                    self.fine_features_grad[:nb_samples,:] = rgb_feat.grad[:,shift_p+6:]  #math.sin(0.5*math.pi*self.loc_iter/self.end_iter_loc)*rgb_feat.grad[:,shift_p+8:] +\
                                                                #(1.0 - math.sin(0.5*math.pi*self.loc_iter/self.end_iter_loc))*coarse_feat.grad[:,shift_p+5:]
                    self.norm_features_grad[:nb_samples,:] = rgb_feat.grad[:,shift_p+3:shift_p+6] + 0.5*coarse_feat.grad[:,shift_p+0:shift_p+3]  #math.sin(0.5*math.pi*self.loc_iter/self.end_iter_loc)*rgb_feat.grad[:,shift_p+5:shift_p+8] +\
            ...
            if self.double_net:     
                ...
                self.grad_sdf = self.vortSDF_renderer_fine.grads_sdf + self.f_w*self.grad_sdf_norm  #+ self.f_w*(self.grad_sdf_net + self.grad_sdf_norm) # +\
            ...
            if iter_step % acc_it == acc_it-1:
                self.fine_features.grad = self.fine_features.grad #+ self.tv_f*self.grad_feat_smooth
            ...
            if iter_step % acc_it == acc_it-1:
                if (iter_step+1) < 2000: 
                    ...
                    self.grad_norm_smooth[self.mask_background[:] == 1.0] = 0.0
                    self.grad_eik[self.mask_background[:] == 1.0] = 0.0
                    self.grad_sdf_smooth[self.mask_background[:] == 1.0] = 0.0

                    """if not iter_step % 5 == 0:
                        self.grad_norm_smooth[self.sdf.grad[:] == 0.0] = 0.0
                        self.grad_eik[self.sdf.grad[:] == 0.0] = 0.0
                        self.grad_sdf_smooth[self.sdf.grad[:] == 0.0] = 0.0
                        self.grad_norm_smooth[:] = 0.0
                ...
                if False: #(iter_step+1) > 70000: 
                    self.sdf.grad = self.norm_grad * self.sdf.grad + (self.e_w*self.grad_eik+\
                                                                    self.s_w*self.grad_norm_smooth+\
            ...
            if (iter_step+1) in up_iters: #== 2000 or (iter_step+1) == 10000 or (iter_step+1) == 30000 or (iter_step+1) == 50000 or (iter_step+1) == 70000:# or (iter_step+1) == 45000:
                ...
                self.tet32.surface_from_sdf(self.sdf.detach().cpu().numpy().reshape(-1), "Exp/{}/meshes/test_tri_up.ply".format(self.data_name), self.dataset.scale_mats_np[0][:3, 3][None], self.dataset.scale_mats_np[0][0, 0])
                ...
                if (iter_step+1) == up_iters[1]:
                    ...
                    self.s_max = 150 #60/(5.0*self.sigma) #200
                    """self.s_w = 1.0e-3
                    self.e_w = 5.0e-4
                    self.tv_w = 1.0e-5"""
                ...
                if (iter_step+1) == up_iters[2]:
                    ...
                    self.f_w = 1.0
                    self.learning_rate = 1e-3
                    self.learning_rate_sdf = 5e-3 #1e-4
                    self.learning_rate_feat = 5e-3 #1.0e-2
                ...
                if (iter_step+1) == up_iters[3]:
                    ...
                    self.e_w = 1.0e-7 #1.0e-6 #1.0e-9 #1.0e-7 #5.0e-3
            ...
            if (iter_step+1) % self.report_freq == 0:
                ...
                print('iter:{:8>d} s_w = {}, e_w = {}, tv_w = {}, nb_samples={}, sigma={}, lr={}'.format(iter_step, self.s_w, self.e_w, self.tv_w, nb_samples, self.sigma, self.optimizer_sdf.param_groups[0]['lr']))
            ...
            if (iter_step+1) % self.val_freq == 0:
                ...
                self.tet32.surface_from_sdf(self.sdf.detach().cpu().numpy().reshape(-1), "Exp/{}/meshes/test_tri_{}.ply".format(self.data_name, iter_step), self.dataset.scale_mats_np[0][:3, 3][None], self.dataset.scale_mats_np[0][0, 0])
                #if iter_step % 3 == 0:
                """self.activated[:] = 1
                with torch.no_grad():
                    self.sdf_smooth[:] = 0.0
                backprop_cuda.knn_smooth(self.sites.shape[0], 96, self.sigma, self.sigma_feat, 1, self.sites,  self.activated,
                                        self.grad_sdf_space, self.sdf, self.fine_features, self.tet32.knn_sites, self.sdf_smooth)
                """
                self.tet32.surface_from_sdf(self.sdf_smooth.detach().cpu().numpy().reshape(-1), "Exp/{}/meshes/test_tri_smooth.ply".format(self.data_name), self.dataset.scale_mats_np[0][:3, 3][None], self.dataset.scale_mats_np[0][0, 0])                               
                with torch.no_grad():
                    self.sdf[ext_flag[:] == 1] = -1000.0
    ...
    def render_image(self, img_idx = 0, iter_step = 0, resolution_level = 1):
        ...
        for rays_o_batch, rays_d_batch in zip(rays_o.split(self.batch_size), rays_d.split(self.batch_size)):
            ...
            if self.double_net:
                coarse_feat = torch.cat([xyz_emb, viewdirs_emb, self.out_sdf[:nb_samples,:2], self.out_grads[:nb_samples,:3], self.out_feat[:nb_samples,:32]], -1)       
                colors_feat = self.color_coarse.rgb(coarse_feat)   
                self.colors = torch.sigmoid(colors_feat)
                self.colors = self.colors.contiguous()
    ...
    def load_checkpoint(self, checkpoint_name):
        ...
        if (self.iter_step+1) == 10000:
            ...
            self.e_w = 5.0e-4
            self.tv_w = 1.0e-5"""
            self.s_w = 0.0#1.0e-2 #1e-6
            self.e_w = 1.0e-4#1.0e-8 #5.0e-3
            self.tv_w = 0.0#5.0e-3 #1.0e-8 #1.0e-1
            self.tv_f = 1.0e-8
            self.f_w = 1.0 #1.0
        ...
        if (self.iter_step+1) == 70000:
            ...
            self.s_max = 2000
            self.s_w = 5.0e-5 #1.0e-5 
            self.e_w = 5.0e-6 #1.0e-7
            self.tv_w = 1.0e-5 #5.0e-6 
            self.tv_f = 1.0e-5
            self.end_iter_loc = 20000
            self.learning_rate = 5e-4
            self.learning_rate_sdf = 5.0e-5
