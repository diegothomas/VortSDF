...
class Runner:
    ...
    def train(self, data_name, K_NN = 24, verbose = True, is_continue = False):
        ...
        for iter_step in tqdm(range(self.end_iter)):
            ...
            if self.double_net:
                if self.position_encoding:
                    ...
                    self.fine_features_grad[:nb_samples,:] = math.sin(0.5*math.pi*self.loc_iter/self.end_iter_loc)*rgb_feat.grad[:,shift_p+8:] +\
                                                                (1.0 - math.sin(0.5*math.pi*self.loc_iter/self.end_iter_loc))*coarse_feat.grad[:,shift_p+5:]
                    self.norm_features_grad[:nb_samples,:] = math.sin(0.5*math.pi*self.loc_iter/self.end_iter_loc)*rgb_feat.grad[:,shift_p+5:shift_p+8] +\
            ...
            if (iter_step+1) == 2000 or (iter_step+1) == 10000 or (iter_step+1) == 30000 or (iter_step+1) == 50000 or (iter_step+1) == 70000:# or (iter_step+1) == 45000:
                ...
                if (iter_step+1) == 2000:
                    ...
                    self.s_start = 30/(10.0*self.sigma) #50.0
                    self.s_max = 60/(5.0*self.sigma) #200
        ...
        #self.tet32.make_clipped_CVT(self.sdf.detach(), self.sigma, self.grad_sdf_space, self.visual_hull,  "Exp/{}/final_CVT.obj".format(self.data_name), self.dataset.scale_mats_np[0][:3, 3][None], self.dataset.scale_mats_np[0][0, 0])
        ...
        #self.tet32.make_clipped_CVT(self.sdf_smooth, self.sigma, self.grad_sdf_space, self.visual_hull, "Exp/{}/final_CVT_smooth.obj".format(self.data_name), self.dataset.scale_mats_np[0][:3, 3][None], self.dataset.scale_mats_np[0][0, 0])
    ...
    def load_checkpoint(self, checkpoint_name):
        ...
        if (self.iter_step+1) == 10000:
            ...
            self.tv_w = 5.0e-3 #1.0e-8 #1.0e-1
            self.tv_f = 1.0e-8
            self.f_w = 1.0 #1.0
            self.learning_rate = 5e-4
            self.learning_rate_sdf = 5.0e-4 #3.0e-4 #5
